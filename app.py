...
def status():
    ...
    return render_template_string(open('templates/status.html').read(), rows=rows, all_approved=all_approved)
...
def admin_login():
    ...
    return render_template_string(open('templates/admin.html').read(), rows=rows, churches=churches, areas=areas, erase_pass=ERASE_PASSWORD)
