        ...
        for name in names:
            ...
            if cursor.fetchone()[0] > 0:
                return jsonify({
                    ...
                    'error',
                    'message':
                    f"The name '{name}' has already been registered."
                })
...
def admin_login():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute(
            'SELECT full_name, church, area, status FROM registrations')
        rows = cursor.fetchall()
            ...
            <table border="1" cellpadding="10">
                ...
                {% for full_name, church, area, status in rows %}
                    <tr data-church="{{ church }}" data-area="{{ area }}">
                        ...
                        <td>{{ church }}</td>
                        <td>{{ area }}</td>
                        <td style="background-color: {{ 'green' if status == 'Approved' else 'red' }}; color: white;">
                            {{ status }}
                            {% if status == 'Pending' %}
...
def approve():
    ...
    with sqlite3.connect(DB_FILE) as conn:
        conn.execute(
            'UPDATE registrations SET status = "Approved" WHERE full_name = ?',
            (full_name, ))
    return '', 204
